...
describe('Quiz Component', () => {
  it('showNextQuestionHandlerHandler() method increments the quiz_position by 1 @quiz-show-next-question-increments-position-state', () => {
    ...
    if (quiz.state().quiz_position) {
      ...
      try {
        quiz.instance().showNextQuestionHandlerHandler()
      } catch (e) {
        assert(false, "There's not a method named `showNextQuestionHandlerHandler()` in the Quiz class.")
      }
      assert(quiz.state().quiz_position == prev_position + 1, "The Quiz component state's `quiz_position` value is not being incremented by 1 when the `showNextQuestionHandlerHandler()` method is called.")
    }
  })
})